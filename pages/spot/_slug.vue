	...
	<section class="spot">
		...
		<footer class="spot-complementary">
			<div class="spot-actions">
				...
				<button class="btn btn--icon" :data-tooltip="this.$store.state.lang.spot.actions.edit" @click="editSpot()">
			...
			<div class="spot-infos">
				<div class="spot-date">{{ this.$store.state.lang.spot.infos.created }} <time>{{ spot.createdAt | moment('DD-MM-YYYY') }}</time></div>
				<div class="spot-date" v-if="spot.modifiedAt !== spot.createdAt">{{ this.$store.state.lang.spot.infos.edited }} <time>{{ spot.modifiedAt | moment('DD-MM-YYYY') }}</time></div>
			</div>
		</footer>
	</section>
</template>
...
export default {
	...
	async asyncData({ params, app }) {
		let { data } = await app.$axios.get(
			`${process.env.api.spots}/${params.slug}`
		);
	...
	methods: {
		...
		removeSpot() {
			this.$modal.show('dialog', {
				...
				buttons: [
					{
						title: this.$store.state.lang.modal.spot.remove.action.buttons
							.cancel
					},
					{
						title: this.$store.state.lang.modal.spot.remove.action.buttons
		...
		confirmRemoveSpot() {
			...
			that.$axios
				.delete(`${process.env.api.spots}/${this.spot._id}`)
				.then(res => {
					...
					that.$modal.show('dialog', {
						...
						buttons: [
							{
								title:
									that.$store.state.lang.modal.spot.delete.success.buttons.close
							}
						]
					});
				})
				.catch(error => {
					that.$modal.show('dialog', {
						...
						buttons: [
							{
								title:
									that.$store.state.lang.modal.spot.delete.error.buttons.close
							}
						]
					});
				});
	...
	head() {
		return {
			title: `${this.spot.title}, ${this.spot.location.city} (${
				this.spot.location.country
			})`,
			htmlAttrs: {
